...
Usage:
python q25_tp.py --model_path ../weights --prompt "Hello, how are you?" --max_tokens 20 --tp 4 --dp 1
"""
import os
import sys
import time
    ...
    def __call__(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, cos=None, sin=None):
        ...
        if past_key_value is not None:
            ...
            if past_k.shape[1] == self.num_kv_heads and past_k.shape[2] != self.num_kv_heads:
                # Past cache is in transposed format, transpose back
                past_k = jnp.transpose(past_k, (0,2,1,3))
                past_v = jnp.transpose(past_v, (0,2,1,3))
            
            # Handle head count differences for GQA
            if past_k.shape[2] == self.num_heads:  # Past cache in query head format
...
# --- Keep original functions from q25_jax.py ---
...
def main():
    ...
    if mesh is not None:
        ...
        # Fall back to single-device loading (import from original)
