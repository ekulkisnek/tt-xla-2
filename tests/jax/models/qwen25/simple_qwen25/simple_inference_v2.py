#!/usr/bin/env python3
"""
Streamlined Qwen2.5-7B inference script for single-device JAX.
Minimal code, maximum performance.

Usage: python simple_inference_v2.py --model_path ../weights --prompt "Hello" --max_tokens 20
"""
import os
import json
import gc
...
def generate_text(model, params, tokenizer, prompt, max_tokens, temperature=0.7):
    ...
    for _ in range(max_tokens):
        ...
        if temperature < 1e-5:
            ...
            next_token = jax.random.categorical(rng_key, logits[0, -1, :] / temperature)
        
        # Update state
        state["input_ids"] = jnp.array([[int(next_token)]], dtype=jnp.int32)
