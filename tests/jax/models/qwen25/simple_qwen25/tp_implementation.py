...
Usage:
python tp_implementation.py --model_path ../weights --prompt "Hello, how are you?" --max_tokens 20 --tp 4 --dp 1
"""
import os
import sys
import time
...
# === ORIGINAL FUNCTIONS FROM q25_jax.py ===
...
class QwenAttention(nn.Module):
    ...
    def setup(self):
        ...
        # TP: Q/K/V sharded on output, O sharded on input
        self.q_proj = TensorParallelDense(
            features=self.hidden_size, dtype=self.dtype, use_bias=False,
            shard_axes=(None, "model"), name="q_proj"
        )
        self.k_proj = TensorParallelDense(
            features=self.kv_dim, dtype=self.dtype, use_bias=False,
            shard_axes=(None, "model"), name="k_proj"
        )
        self.v_proj = TensorParallelDense(
            features=self.kv_dim, dtype=self.dtype, use_bias=False,
            shard_axes=(None, "model"), name="v_proj"
        )
        self.o_proj = TensorParallelDense(
            features=self.hidden_size, dtype=self.dtype, use_bias=False,
            shard_axes=("model", None), name="o_proj"
        )
...
class QwenMLP(nn.Module):
    ...
    def setup(self):
        ...
        # TP: Gate/Up sharded on output, Down sharded on input
        self.gate_proj = TensorParallelDense(
            features=self.intermediate_size, dtype=self.dtype, use_bias=False,
            shard_axes=(None, "model"), name="gate_proj"
        )
        self.up_proj = TensorParallelDense(
            features=self.intermediate_size, dtype=self.dtype, use_bias=False,
            shard_axes=(None, "model"), name="up_proj"
        )
        self.down_proj = TensorParallelDense(
            features=self.hidden_size, dtype=self.dtype, use_bias=False,
            shard_axes=("model", None), name="down_proj"
        )
...
# === ORIGINAL WEIGHT LOADING FUNCTIONS ===
...
def get_partition_spec(param_name: str) -> P:
    ...
    # TP sharding rules:
    # - q_proj, k_proj, v_proj, gate_proj, up_proj → P(None, "model")  (output dim TP)
    # - o_proj, down_proj                           → P("model", None) (input dim TP)
    # - embed_tokens, lm_head                       → P(None, None)     (replicated)
    ...
    if any(proj in param_name for proj in ["q_proj", "k_proj", "v_proj", "gate_proj", "up_proj"]):
        ...
        return P(None, None)    # Everything else replicated
...
def process_safetensors_file(file_path, dtype, mesh=None):
    """Process safetensors file with optional tensor parallel sharding."""
    flax_params = {"params": {}}
    unmapped_keys = []
...
def load_params(model, model_path, dtype, mesh=None):
    """Load model parameters with optional tensor parallel sharding."""
    logger.info("Loading weights...")
    if mesh:
        logger.info("Loading with tensor parallel sharding")
    ...
    # Phase 1 validation: Check sharding
    if mesh is not None:
        ...
        logger.info("✓ Parameter sharding validation complete")
...
def generate_step(model_apply, params, input_ids, attention_mask, position_ids, past_key_values):
    """JIT-compiled generation step with donate_argnums."""
    outputs = model_apply(
        params,
        input_ids=input_ids,
        attention_mask=attention_mask,
...
def generate_text(model, params, tokenizer, prompt, max_tokens, temperature=0.7):
    ...
    # JIT-compiled apply function
    model_apply = jax.jit(model.apply, donate_argnums=(0,))
    
    # Generate tokens
    for _ in range(max_tokens):
        # Forward pass (with JIT and donation for Phase 2)
        logits, past_key_values = generate_step(
            model_apply, params,
            state["input_ids"], state["attention_mask"],
