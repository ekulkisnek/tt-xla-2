#!/usr/bin/env python3
"""
Real Qwen 2.5-7B with Tensor Parallelism - FIXED VERSION
Based on the working q25_jax.py model structure
"""
...
# Add tensor parallel imports
from jax.sharding import NamedSharding, PartitionSpec
from jax.experimental import mesh_utils

# Logging setup
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger("real_qwen25_tp_fixed")
...
class TensorParallelDense(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        if self.shard_axes != (None, None):
            ...
            kernel = jax.lax.with_sharding_constraint(kernel, NamedSharding(jax.experimental.mesh.current_mesh() or create_mesh(), shard_spec))
        ...
        # Apply reduce-scatter if specified
        if self.reduce_scatter and jax.experimental.mesh.current_mesh() is not None:
            y = jax.lax.psum(y, axis_name='model')
        
        return y

# === COPY THE WORKING MODEL STRUCTURE FROM q25_jax.py ===
...
class QwenAttention(nn.Module):
    config: Dict[str, Any]
    dtype: jnp.dtype = jnp.float32
    use_tp: bool = False  # Whether to use tensor parallelism
    ...
    def __call__(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, cos=None, sin=None):
        ...
        # Handle past key-value cache (copy from working q25_jax.py)
...
# === COPY WEIGHT LOADING FROM q25_jax.py ===
...
def load_params(model, model_path, dtype):
    ...
    # Skip weight tying (following q25_jax.py)
    logger.info("✅ Weights loaded successfully (no weight tying)")
    
    return params

# === COPY GENERATION FROM q25_jax.py ===
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Real Qwen2.5-7B with Tensor Parallelism - FIXED")
    ...
    if args.use_tp:
        ...
        print("🔥 Using Single Device (like working q25_jax.py)")
