...
import qwen_nnx
import qwen_nnx.generate
from qwen_nnx.util import timer
from .model import Qwen25ForCausalLM, Axis


MODEL = "../qwen25-7b/qwen25_7b_instruct_weights"  # Or local path
...
def mesh() -> jax.sharding.Mesh:
    devices = jax.devices()
    return jax.sharding.Mesh(jnp.array(devices).reshape(-1, 1), ('x',))


@pytest.fixture(scope="module")
def nnx_model(mesh: jax.sharding.Mesh) -> nnx.Module:
    with timer("NNX model loading"):
        return Qwen25ForCausalLM.load(
            MODEL,
            dtype=jnp.float32,
            mesh=mesh,
...
def load_hf_model() -> transformers.Qwen2ForCausalLM:
...
def test_generate(tokenizer, mesh, nnx_model):
    ...
    generator = qwen_nnx.generate.Generator(nnx_model, max_seqlen=30)
