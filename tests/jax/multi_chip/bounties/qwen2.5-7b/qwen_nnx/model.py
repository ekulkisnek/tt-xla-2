...
Not supported:
- Sliding window (though config has it disabled)
...
class KVCacheLayer:
    ...
    def update(
        ...
        """Update KV cache with next KV pair."""

        B, T, _H, _D = k.shape
        k = jnp.expand_dims(k, axis=1)
        v = jnp.expand_dims(v, axis=1)
        cache_k = jax.lax.dynamic_update_slice(self.cache_k, k, [0, self.index, 0, 0])
        cache_v = jax.lax.dynamic_update_slice(self.cache_v, v, [0, self.index, 0, 0])
        index = self.index + T
        return KVCacheLayer(cache_k=cache_k, cache_v=cache_v, index=index)
...
def create_sin_cos(length: int, dim: int, base: float = 10000.0) -> Array:
    """Creates sin and cos for rotary embedding."""

    inv_freq = 1.0 / (base ** (jnp.arange(0, dim, 2, dtype="float32") / dim))
...
class Attention(nnx.Module):
    """Qwen2.5 attention supports different number of Q heads vs KV heads and Q/V biases."""
    ...
    def __call__(self, x: Float[Array, "B S E"], attention_mask: Optional[Array] = None, position_ids: Optional[Array] = None, cache: Optional[KVCacheLayer] = None, return_kv: bool = False) -> Float[Array, "B S E"] | tuple[Float[Array, "B S E"], tuple[Array, Array]] | tuple[Float[Array, "B S E"], KVCacheLayer]:
        """Full forward pass with optional cache for generation."""
        B, S, _ = x.shape
        ...
        if cache is not None:
            ...
            xq = xq.reshape(B, S, self.n_q_heads, self.head_dim)
            xk = xk.reshape(B, S, self.n_kv_heads, self.head_dim)
            xv = xv.reshape(B, S, self.n_kv_heads, self.head_dim)
            
            cache = cache.update(xk, xv)
            xk = cache.cache_k
            xv = cache.cache_v
            index = jnp.array([cache.index - 1])
            xq, xk = apply_rotary_embedding(xq, xk, None, None, decode=True, rotary_index=index)
            out = jax.nn.dot_product_attention(xq, xk, xv, is_causal=True)
            # Keep shape (B, S, n_q_heads, head_dim) for wo layer
            if return_kv:
            ...
            cos, sin = generate_fixed_pos_embedding(self.head_dim, S)
            xq, xk, xv = self.wq(x), self.wk(x), self.wv(x)
            ...
            # Use the provided attention_mask if it exists and is correctly shaped (B, 1, S, S)
            mask = attention_mask if attention_mask is not None and attention_mask.ndim == 4 else None
            out = jax.nn.dot_product_attention(xq, xk, xv, mask=mask, is_causal=True)
...
class TransformerBlock(nnx.Module):
    ...
    def __call__(self, x: Float[Array, "B S E"], attention_mask: Optional[Array] = None, position_ids: Optional[Array] = None, cache: Optional[KVCacheLayer] = None, return_kv: bool = False) -> Float[Array, "B S E"] | tuple[Float[Array, "B S E"], tuple[Array, Array]] | tuple[Float[Array, "B S E"], KVCacheLayer]:
        ...
        attn_out = self.attention(norm_x, attention_mask, position_ids, cache, return_kv=return_kv)
        if return_kv:
...
class QwenModel(nnx.Module):
    ...
    def __call__(self, input_ids: Integer[Array, "B S"], attention_mask: Optional[Array] = None, position_ids: Optional[Array] = None, cache: Optional[KVCache] = None, return_kv: bool = False) -> Float[Array, "B S V"] | tuple[Float[Array, "B S V"], Optional[KVCache]] | tuple[Float[Array, "B S V"], list[tuple[Array, Array]]]:
        """Full forward pass with optional cache for generation."""
        B, S = input_ids.shape
        ...
        causal_mask = make_causal_mask(input_ids)  # (B,1,S,S)
        padding_mask = (attention_mask > 0)[:, None, None, :]  # (B,1,1,S)
        mask = (causal_mask * padding_mask) > 0

        h = self.embed(input_ids)
        updated_cache_layers = []
        updated_kvs = []
        for i, layer in enumerate(self.layers):
            cache_layer = None if cache is None else cache.layers[i]
            h = layer(h, mask, position_ids, cache_layer, return_kv=return_kv)
            if return_kv:
                h, kv = h
                updated_kvs.append(kv)
            if isinstance(h, tuple):
                h, updated_cache_layer = h
                updated_cache_layers.append(updated_cache_layer)
        ...
        if cache is not None or updated_cache_layers:
            # If we were using cache or have updated cache layers, return tuple
            updated_cache = KVCache(updated_cache_layers) if updated_cache_layers else None
            return logits, updated_cache
        else:
            # Simple forward pass, just return logits
            return logits
...
def _load_hf_pt_params(hf_model: str, abs_state: nnx.State) -> nnx.State:
    ...
    with ExitStack() as stack:
        ...
        def reshape_bias(param: jax.Array, abs_param):
            return param.reshape(abs_param.shape)
...
def _load_orbax(ckpt_dir: Path, abs_state: nnx.State) -> nnx.State:
    ...
    with ocp.StandardCheckpointer() as checkpointer:
        return checkpointer.restore(ckpt_dir.absolute(), item=abs_state) 